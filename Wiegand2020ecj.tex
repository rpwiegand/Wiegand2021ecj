...
\begin{figure}[t]
  \center\begin{tikzpicture}[scale=1]
...
%
\begin{figure}[ht]
  %\center\includegraphics[width=1\textwidth]{Figures/bounded-500-viz-mu2lam8.pdf}
...
See Figure~\ref{fig:local:conv} for an illustration.  The space on the right will have larger cover than the space on the left, but neither can admit more points.  This example is easy to generalize to larger $k$ and different packings of archives and $\rho_{min}$ values.  It can also be seen empirically with the fact that not every run of our experiments above converged to the same final cover value.  
%
\begin{figure}[ht]
